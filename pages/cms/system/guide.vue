        ...
        <el-table v-if="docs?.items" table-layout="auto" fit :data="docs.items" style="width: 100%">
          ...
          <el-table-column fixed="right" label="操作">
            <template #default="{ row }">
              ...
              <el-button v-if="row.status !== 3" plain text size="small" type="warning" @click="handleDialog(row, 'edit')">
                编辑
              </el-button>
              <el-button v-if="row.status !== 0 && row.status !== 3" plain text size="small" type="danger" @click="handleArchiveDoc(row.id, row)">
                归档
              </el-button>
              <el-button v-if="row.status === 1" plain text size="small" type="primary" @click="handlePublishVersion(row.id, row)">
                发版
              </el-button>
            </template>
          </el-table-column>
      ...
      <teleport to="body">
        <div class="GuideEditor" :class="{ visible: dialogOptions.visible }">
          ...
          <div v-if="dialogOptions.data" class="GuideContent">
            ...
            <ArticleThEditor v-model="dialogOptions.data.content!" :readonly="dialogOptions.mode === 'read' || dialogOptions.data.status === 3" @save="tryTempSave">
              <template #property>
                <el-form
